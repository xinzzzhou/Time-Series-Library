...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.in_seg_num = self.pad_in_len // self.seg_len + 1
        self.out_seg_num = ceil(self.in_seg_num / (self.win_size ** (configs.e_layers - 1)))
        self.head_nf = configs.d_model * self.out_seg_num

        # Embedding
        self.enc_value_embedding = PatchEmbedding(configs.d_model, self.seg_len, self.patch_stride, self.pad_in_len - configs.seq_len, 0)
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        # if x_enc.shape[1] < self.enc_in:
        #     x_enc += self.enc_pos_embedding[:,:200,:,:]
        # else:
        #     x_enc += self.enc_pos_embedding   
        x_enc += self.enc_pos_embedding
        x_enc = self.pre_norm(x_enc)
        enc_out, attns = self.encoder(x_enc)  #2-7GB memory (2 bs)

        dec_in = repeat(self.dec_pos_embedding, 'b ts_d l d -> (repeat b) ts_d l d', repeat=x_enc.shape[0])  #7-14GB memory (2 bs)
        dec_out = self.decoder(dec_in, enc_out)
        return dec_out


    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        dec_out = self.forecast(x_enc, x_mark_enc, x_dec, x_mark_dec) #memory
