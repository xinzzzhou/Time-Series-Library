...
layer=3
learning_rate=0.05 # learning_rates=(0.1 0.01 0.0001)
batch_size=16 # batch_sizes=(16 32 64 128)
  ...
  # for d_ff in "${d_ffs[@]}"; do
    ...
    #         for n_heads in "${n_headss[@]}"; do
              ...
              nohup python -u $root_directory/baselines/Time-Series-Library/run_self_traffic.py \
                ...
                --dropout 0.2\
                --freq 'h' > $root_directory/HTSFB_output/"($model_name)"_traffic336_"($horizon)"_"($current_time)"_"($d_ff)"_"($d_model)"_"($layer)"_"($learning_rate)"_"($batch_size)"_"($n_heads)".out
    #         done
    #       done
    #     done
    #   done
