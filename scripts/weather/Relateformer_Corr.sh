    ...
    # for d_ff in "${d_ffs[@]}"; do
      ...
      #         for n_heads in "${n_headss[@]}"; do
              ...
              nohup python -u $root_directory/baselines/Time-Series-Library/run_self.py \
                ...
                --freq 'm' > $root_directory/HTSFB_output/"($model_name)"_crime_24_"($horizon)"_"($current_time)"_"($corr)"_"($d_ff)"_"($d_model)"_"($layer)"_"($learning_rate)"_"($batch_size)"_"($n_heads)".out
    #           done
    #         done
    #       done
    #     done
